# 🛡️ Production-Grade DevOps Case Study: Spring Petclinic on Kubernetes

Welcome to the **DevOps Case Study** repository. This project showcases a fully automated, secure, highly available, and production-ready infrastructure ecosystem designed to deploy and run the classic **Spring Petclinic** Java application on a Kubernetes cluster. 

The goal of this case study is to demonstrate enterprise-grade DevOps workflows, incorporating containerization best practices, infrastructure orchestration, strict security controls, declarative configuration management, automated database operations, and continuous integration/continuous deployment (CI/CD) pipelines.

---

## 🗺️ Architectural Overview

This system is engineered for local development and validation using **Kind (Kubernetes in Docker)** with a local registry, simulating a real-world enterprise cloud environment.
...
Below is the directory structure highlighting the role of each component:
...
├── Jenkinsfile                   # Enterprise-ready CI/CD Pipeline
├── app/
│   └── spring-petclinic/         # Main Spring Boot Application Source Code
├── docker/
│   └── Dockerfile                # Optimized multi-stage, secure JRE Dockerfile
├── compose/
│   ├── docker-compose.yml        # Multi-container orchestration (App + Postgres) using Secrets
│   └── jenkins-controller.yml    # Runs Jenkins Controller for CI/CD tasks
├── k8s/
│   ├── kind-cluster.yaml         # Custom Kind configuration with Ingress & Port Mapping
│   └── base/                     # Declarative Kubernetes Manifests (Kustomize Base)
│       ├── namespace.yaml        # Prod namespace enforcing strict Pod Security Standards
│       ├── kustomization.yaml    # Declarative composition of resources
│       ├── resourcequota.yaml    # Hard resource bounds per namespace
│       ├── limitrange.yaml       # Default request/limits allocations for containers
│       ├── configmap.yaml        # Non-sensitive app configurations
│       ├── app-deployment.yaml   # Hardened application pods
│       ├── app-service.yaml      # NodePort service exposing app
│       ├── app-ingress.yaml      # Ingress rules routing / to the service
│       ├── app-hpa.yaml          # Horizontal Pod Autoscaler for traffic spikes
│       ├── app-pdb.yaml          # PodDisruptionBudget ensuring high availability
│       ├── app-networkpolicy.yaml# Limits app pod communications
│       ├── postgres-service.yaml # Headless service for DB
│       ├── postgres-statefulset.yaml # Stateful db workload with volume persistent claim
│       ├── postgres-networkpolicy.yaml # Restricts DB ingress strictly to app and backup
│       ├── postgres-backup-pvc.yaml # Storage volume for database backups
│       └── postgres-backup-cronjob.yaml # Cronjob executing automated daily backups
└── scripts/                      # Automated helper PowerShell scripts
    ├── init-secrets.ps1          # Locally generates random credentials
    ├── create-cluster.ps1        # Simple Kind cluster setup
    ├── create-cluster-with-registry.ps1 # Setup Kind cluster + connects local registry
    ...
    ├── build-image.ps1           # Builds and Pushes App Image
    ├── deploy-k8s.ps1            # Automates secret management and deploys resources
    └── destroy-k8s.ps1           # Full cleanup utility
```
...
### 1. Secure & Optimized Dockerization
*   **Multi-Stage Builds:** Minimizes image size and eliminates compile-time overhead inside production. Build stage uses JDK 21, runtime stage uses lightweight, optimized JRE 21.
*   **Reproducible Baseline:** Base images are pinned to Ubuntu-based Jammy versions (`eclipse-temurin:21-jdk-jammy` & `eclipse-temurin:21-jre-jammy`).
...
*   **Pod Security Standards (PSS):** Enforces `baseline` security policies and audits `restricted` rules at the namespace level via label directives.
*   **Security Context Hardening:** Application pods feature a read-only root filesystem (`readOnlyRootFilesystem: true`), dropped Linux capabilities (`ALL`), blocked privilege escalation, and runtime-default seccomp profile configuration.
...
*   **Traffic Management & Scaling:** 
    *   **Horizontal Pod Autoscaler (HPA)** automatically scales application replicas dynamically up to 3 when CPU utilization crosses 70%.
    *   **Pod Disruption Budget (PDB)** prevents maintenance operations from bringing down all app replicas at once (`minAvailable: 1`).
...
### 3. GitOps & Secrets Security
*   **Secrets Configuration:** Application and DB are fully configured using file-based config trees and secrets. No raw passwords or JDBC strings are hardcoded in the manifests.
*   **Automated Generation:** Local PowerShell scripts create secure, cryptographically random, gitignored credentials.

### 4. Fully Automated CI/CD (Jenkins)
*   **Integrity Assurance:** The pipeline automatically tests the codebase using the Maven wrapper before any build operations occur.
...
*   **Declarative Infrastructure:** Optional platform step dynamically updates the local cluster configurations, installing the Inginx Ingress controller and Metrics Server dynamically.
...
```
*This configures a custom multi-node ready Kind cluster mapping ports 80 -> 8080 and 443 -> 8443 on your local machine, and connects it to a local registry container running on `localhost:5001`.*
...
Apply the hardened configurations and wait for deployments to stabilize:
```powershell
...
# 🛡️ Üretim Standartlarında DevOps Örnek Çalışması: Kubernetes Üzerinde Spring Petclinic

**DevOps Case Study** (Örnek Çalışma) deposuna hoş geldiniz. Bu proje; klasik **Spring Petclinic** Java uygulamasını güvenli, yüksek düzeyde erişilebilir ve üretim standartlarına (production-ready) uygun olarak Kubernetes kümesinde çalıştırmak üzere tasarlanmış, tamamen otomatik bir altyapı ekosistemini sunar.

Bu çalışmanın amacı; en iyi konteynerleştirme pratiklerini, altyapı yönetimini, sıkı güvenlik denetimlerini, deklaratif yapılandırma yönetimini, otomatik veritabanı işlemlerini ve sürekli entegrasyon/sürekli dağıtım (CI/CD) süreçlerini barındıran kurumsal düzeydeki DevOps iş akışlarını göstermektir.

---

## 🌟 Öne Çıkan Teknik Özellikler

### 1. Güvenli & Optimize Edilmiş Docker Yapılandırması
*   **Çok Aşamalı Yapı (Multi-stage Build):** Derleme bağımlılıklarını nihai paketten uzak tutarak image boyutunu küçültür. Derleme aşamasında JDK 21, runtime aşamasında ise hafif ve optimize edilmiş JRE 21 kullanılır.
*   **Kararlı ve Sabit Altyapı:** Base image'ler, tutarlı build garantisi sağlamak için Ubuntu tabanlı Jammy versiyonlarına sabitlenmiştir (`eclipse-temurin:21-jdk-jammy` & `eclipse-temurin:21-jre-jammy`).
*   **Katman Optimizasyonu:** Ardışık komutlar tek satırda birleştirilerek (`RUN chmod +x ... && ./mvnw ...`) gereksiz katman oluşumu engellenmiş ve cache performansı artırılmıştır.
*   **Non-Root Güvenliği:** Konteyner içerisinde özel yetkisiz kullanıcı ve grup (`petclinic` UID/GID `10001`) tanımlanmıştır. Uygulama yetkisiz kullanıcı ile çalışarak olası konteyner sızma zafiyetlerini (container breakout) engeller.

### 2. Gelişmiş Kubernetes Mimarisi
*   **Pod Güvenlik Standartları (PSS):** Namespace düzeyinde etiketler aracılığıyla `baseline` güvenlik politikaları zorunlu kılınmış, `restricted` kuralları denetlenmiştir.
...
*   **Kesintisiz Güncelleme (Zero-Downtime):** Rolling Update stratejisi ile yeni versiyon dağıtımlarında uygulama kesintisi sıfıra indirgenmiştir (`maxUnavailable: 0` ve `maxSurge: 1`).
*   **Yatay Ölçekleme ve Yüksek Erişilebilirlik:**
...
*   **Sıfır Güven Ağ Politikaları (NetworkPolicies):** Ağ trafiği katı kurallarla izole edilmiştir:
    *   PostgreSQL podları **YALNIZCA** uygulama podlarından ve yedekleme (backup) işinden gelen bağlantıları kabul eder.
...
### 3. GitOps & Güvenli Şifre Yönetimi
*   **Secrets Yapılandırması:** Uygulama ve veritabanı şifreleri Kubernetes Secret'ları üzerinden güvenli ve deklaratif olarak enjekte edilir. Kodlarda veya manifest dosyalarda hiçbir şifre veya bağlantı dizesi açıkta bulunmaz.
*   **Otomatik Üretim:** Yerel PowerShell scriptleri ile git tarafından takip edilmeyen, kriptografik olarak güvenli rastgele şifreler üretilir.

### 4. Tam Otomatize CI/CD (Jenkins Pipeline)
*   **Derleme Testleri:** Pipeline, herhangi bir build işlemine başlamadan önce Maven aracılığıyla uygulamanın birim (unit) testlerini çalıştırarak kod bütünlüğünü doğrular.
...
*   **Dağıtım Doğrulama:** Deploy sonrasında, dağıtımın başarıyla tamamlandığı doğrulanana kadar beklenir ve kümedeki tüm kaynakların güncel durumu ekrana basılır.
...
.\scripts\init-secrets.ps1
```
*Bu komut, repo dışında tutulan (gitignored) `secrets/` klasörünü oluşturur ve içerisine rastgele şifreler kaydeder.*
...
Çok aşamalı Maven build işlemini tetikleyin ve derlenen imajı yerel kayıt defterine yükleyin:
```powershell
.\scripts\build-image.ps1
```
...
## 🧹 Kümenin Temizlenmesi (Kapatılması)
